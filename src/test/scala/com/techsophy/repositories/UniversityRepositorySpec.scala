...
class UniversityRepositorySpec extends AnyFunSuite with UniversityRepository with H2DBComponent with ScalaFutures {

  implicit val defaultPatience = PatienceConfig(timeout = Span(5, Seconds), interval = Span(500, Millis))

  test("create on university table") {
    val response = create(University("hcu", "hyderbad",Some(1)))
    whenReady(response) { id =>
      assert(id.get === 2)
    }
  }
